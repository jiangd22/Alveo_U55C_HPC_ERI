...
#include "shell_pairs_hls.h"



void hrr_ssss(shell_pairs_hls ab, shell_pairs_hls cd,  double* I_) {
        ...
        for (int idy = 0; idy < ncd; idy++) {
            ...
            for (int pab = 0; pab < mab; pab++) {
                for (int pcd = 0; pcd < mcd; pcd++) {
                    ...
                    double zab = ab.Z[(int)iab];
                    double zcd = cd.Z[(int)icd];
                    ...
                    double K = zabcd_inv_sqrt * (ab.K[(int)iab]) * cd.K[(int)icd];
