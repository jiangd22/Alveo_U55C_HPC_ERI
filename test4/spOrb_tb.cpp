...
int main() {
    ...
    int lc = 0;
    int ld = 0; // angular momentum
    int nc = 0;
    int nd = 0; // number of shells
    int mc = 0;
    int md = 0; // number of primitives
    std::array<double, 6> cdZ = {6.342, 6.29243, 13.0011, 6.33731, 6.94737, 6.4369}; // zeta_a + zeta_b
    std::array<double, 6> cdZA = {6.342, 6.29243, 13.0011, 6.33731, 6.94737, 6.4369}; // zeta_a
    std::array<double, 6> cdK = {0.110144, 0.00938326, 0.00124135, 0.203043, 0.0265948, 0.0490335}; // kappa constant
    std::array<double,6> cdS = {1,}; // Schwarz factor sPrt[(ab|ab)]
    std::array<std::array<int,2>,2> cdidx = {{{ 0, }, { 0, }}}; // shell indices (a and b)
    ...
    double I_[1];
    driver(0, la, lb, na, nb, ma, mb, Z, ZA, K, S, idx, P, PA, AB, lc, ld, nc, nd, mc, md, cdZ, cdZA, cdK, cdS, cdidx, cdP, cdPA, cdAB, I_);
